...
import { upsert } from './upsert.action';

type Category = Omit<CategoryInsert, 'palaistra_id'>;
type Group = Omit<GroupInsert, 'palaistra_id'>;
...
export default function Page() {
  ...
  const [state, action, pending] = React.useActionState(upsert, null);
  ...
  React.useEffect(() => {
    if (state?.ok) {
      router.push(`/${palaistra_id}`);
    }
  }, [state, router, palaistra_id]);

  return (
    <div className='grow'>
      <div className="absolute bottom-16 right-1/2 flex grow translate-x-1/2 justify-between rounded-lg bg-foreground/80 px-16 py-2">
        ...
        <form action={action}>
          ...
          <Button type="submit" disabled={pending}>
            {pending && <Loader2Icon className="mr-2 animate-spin" />} Guardar
            cambios
          </Button>
        </form>
      </div>
      <div
        className={cn("h-[calc(100vh-4rem)]",
          showCalendar && 'w-full md:grid md:grid-cols-3 md:gap-4',
        )}
      >
        <div
          className={cn(
            'h-full overflow-x-hidden overflow-y-scroll p-4',
            !showCalendar && 'mx-auto max-w-xl',
            showCalendar && 'hidden md:block',
          )}
        >
          <div>
